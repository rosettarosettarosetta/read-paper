...
其中τ(·)将大于1的所有值转换为1，IoU(·, ·)计算概率图与分割建议之间的交并比分数。等式（2）中的约束条件确保每个分割建议只能分配给一个类别标签。注意，形成最优匹配时可能有一些分割建议未被选择。
